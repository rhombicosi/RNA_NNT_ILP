...
def listp_construct(a_start,a_end,b_start,b_end,listP):
    ...
    for i in range(a_start, a_end+1):
        for j in range(b_start, b_end+1):           
            ...
            sw_weight = bp_df.loc['WS_cis', MOTIF[j-1] + MOTIF[i-1]]
            hw_weight = bp_df.loc['WH_tran', MOTIF[j-1] + MOTIF[i-1]]
            sh_weight = bp_df.loc['HS_tran', MOTIF[j-1] + MOTIF[i-1]]
            ...
            if not math.isnan(float(hw_weight)):
                listP = listP + f' + {hw_weight} WH({j},{i})'
            if not math.isnan(float(ws_weight)):
                listP = listP + f' + {ws_weight} WS({i},{j})'
            if not math.isnan(float(sw_weight)):
                listP = listP + f' + {sw_weight} WS({j},{i})'
            if not math.isnan(float(hs_weight)):
                listP = listP + f' + {hs_weight} HS({i},{j})'
            if not math.isnan(float(sh_weight)):
                listP = listP + f' + {sh_weight} HS({j},{i})'                            
...
# number of edge2edge non canonical contacts
def listn_construct(a_start,a_end,b_start,b_end,listN):
    ...
    for i in range(a_start, a_end+1):
        for j in range(b_start, b_end+1):           
            ...
            if not math.isnan(float(ww_weight)):                
                if MOTIF[i-1] + MOTIF[j-1] not in CP_list:            
                    listN = listN + f' + {0.5} WW({i},{j})'
                    listN = listN + f' + {0.5} WW({j},{i})'
                if (i == a_start and j == b_end) or (i == a_end and j == b_start):
                    listN = listN + f' + {0.5} WW({i},{j})'
                    listN = listN + f' + {0.5} WW({j},{i})'
                
            if not math.isnan(float(hh_weight)):
                listN = listN + f' + {0.5} HH({i},{j})'
                listN = listN + f' + {0.5} HH({j},{i})'
            if not math.isnan(float(ss_weight)):
                listN = listN + f' + {0.5} SS({i},{j})'
                listN = listN + f' + {0.5} SS({j},{i})'
                
            if not math.isnan(float(wh_weight)):
                listN = listN + f' + WH({i},{j})'
            if not math.isnan(float(hw_weight)):
                listN = listN + f' + WH({j},{i})'
            if not math.isnan(float(ws_weight)):
                listN = listN + f' + WS({i},{j})'
            if not math.isnan(float(sw_weight)):
                listN = listN + f' + WS({j},{i})'
            if not math.isnan(float(hs_weight)):
                listN = listN + f' + HS({i},{j})'
            if not math.isnan(float(sh_weight)):
                listN = listN + f' + HS({j},{i})'                            
                        
    return listN
                            
# listN = listn_construct(a_start,a_end,b_start,b_end,listN)
...
def none_score(a_start, a_end, b_start, b_end ):    
    ...
    for i in range(a_start, a_end+1):
        for j in range(b_start, b_end+1): 
            ...
            if math.isnan(float(hh_weight)):
                OUT.write (f'HH({i},{j}) = 0\n')
                OUT.write (f'HH({j},{i}) = 0\n')

none_score(a_start, a_end, b_start, b_end)
